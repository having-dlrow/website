...
non-production-build: module-check ## Build the non-production site, which adds noindex headers to prevent indexing
	cat /sys/fs/cgroup/cpu,cpuacct/cpu.cfs_quota_us && cat /sys/fs/cgroup/cpu,cpuacct/cpu.shares && cat /sys/fs/cgroup/memory/memory.limit_in_bytes & hugo --cleanDestinationDir --enableGitInfo --environment nonprod
